...
\begin{enumerate}
    \item A distribution of $\Psi$ that come from a selection of ``basis" shapes, including rings, guassians, and exponential decay profiles in velocity-delay space. 
...
\begin{enumerate}
    \item We compare the average training loss of the last five epochs with the average loss of the five before that. If there is no change or the the average is increasing we stop training.
...
\begin{figure*}
    \centering
    \textbf{Vizualizing a more ``complicated" transfer function inversion}
...
The key hyperparameters for the (D)CNN architecture and training procedure are summarized in Table~\ref{tab:hyperparams}. Hyperparameters marked ``Tuned'' were determined experimentally; all others were fixed a priori.}

\begingroup\color{red}
\begin{deluxetable*}{lll}
\tablecaption{Summary of (D)CNN hyperparameters used in this work. Hyperparameters marked ``Tuned'' were determined experimentally; all others were fixed a priori.\label{tab:hyperparams}}
\tablecolumns{3}
\tablehead{\colhead{Hyperparameter} & \colhead{Value} & \colhead{Fixed/Tuned}}
\startdata
\cutinhead{Architecture}
Framework & \texttt{Flux.jl} & Fixed \\
Multi-scale branches & 3 (small, medium, large) & Fixed \\
Small time filter ($k_{ts}$) & $\max(2,\,\lfloor n_T/100\rfloor)$ & Fixed \\
Medium time filter ($k_{tm}$) & $\max(5,\,\lfloor n_T/50\rfloor)$ & Fixed \\
Large time filter ($k_{tl}$) & $\max(7,\,\lfloor n_T/5\rfloor)$ & Fixed \\
Velocity filters ($k_{vs}$, $k_{vm}$, $k_{vl}$) & Analogous formulas with $n_\text{LC}$ & Fixed \\
Channels per branch & 32 & Fixed \\
...
Inception+residual blocks & 2 (1D), 1 (2D) & Fixed \\
\cutinhead{Training}
...
Autostop threshold & $10^{-5}$ & Fixed \\
LR reduction on plateau & Halved (up to $2\times$) & Fixed \\
Post-training ensemble cut & $1\sigma$ from mean validation loss & Tuned \\
Training noise level & $5\times10^{-3}\times$ dynamic range & Fixed \\
Missing data fraction & Up to 10\% & Fixed \\
\enddata
\end{deluxetable*}
\endgroup

\section{Sample 2D transfer functions and trailed spectrograms}\label{sec:sample2D}
\begin{figure*}[!htb]
    \centering
...
\tablecolumns{4}
\tablehead{\colhead{Model} & \colhead{Parameter} & \colhead{Range} & \colhead{Distribution}}
\startdata
\cutinhead{Exponential Decay}
 & No additional free parameters & \nodata & \nodata \\
\cutinhead{Gaussian}
 & Width ($\sigma_t$) & 1--$\min(10,\,\tau_\text{peak})$~days & Uniform \\
\cutinhead{Double-Gaussian}
 & Width ($\sigma_t$) & 1--10~days & Uniform \\
 & Secondary peak ($\tau_2$) & $\tau_\text{peak}$--$3\,\tau_\text{peak}$ & Uniform \\
 & Secondary amplitude scale & 0--$5\,e^{-\tau_2/\tau_\text{peak}}$ & Uniform \\
\cutinhead{Accretion Disk}
 & Characteristic time ($\tau_0$) & 5--25~days & $=\tau_\text{peak}$ \\
 & Temperature index ($b$) & $3/4$ & Fixed \\
\cutinhead{BLR Disk-Wind}
 & Inclination ($i$) & $5^\circ$--$85^\circ$ & Uniform \\
...
\new{\section{2D transfer function parameter ranges}\label{sec:2Dparams}
The parameter ranges for the 2D velocity-delay transfer functions used in training are listed in Table~\ref{tab:2Dparams}. Parameters shared with the 1D models (Table~\ref{tab:1Dparams}) are not repeated; only parameters that are unique to the 2D case or that differ from the 1D values are listed. All 2D models share a common velocity scale parameter $\Psi_{v,\max}$, drawn from a uniform distribution spanning $v_\text{min}/10$ to $3\,v_\text{max}/4$, where $v_\text{min}$ and $v_\text{max}$ are the edges of the velocity grid.}
...
\tablecolumns{4}
\tablehead{\colhead{Model} & \colhead{Parameter} & \colhead{Range (2D)} & \colhead{Notes vs.\ 1D}}
\startdata
All 2D models & Velocity scale ($\Psi_{v,\max}$) & $U(v_\text{min}/10,\;3\,v_\text{max}/4)$ & New velocity parameter \\
\cutinhead{Exponential Decay}
 & Velocity profile & $\propto e^{-|v|/\Psi_{v,\max}}$ & Exponential in velocity \\
\cutinhead{Gaussian}
 & Velocity width ($\sigma_v$) & 100--$\min(2000,\,\Psi_{v,\max})$~km\,s$^{-1}$ & New parameter \\
\cutinhead{Ellipse Ring}
 & Time width ($\sigma_t$) & 1--$\min(10,\,\tau_\text{peak})$~days & Gaussian ring in $(\tau,v)$ \\
 & Velocity width & $U(v_\text{min}/10,\;v_\text{max}/2)$ & New parameter \\
 & Ellipse width & 0.5--5.0 & New parameter \\
 & Time center ($\tau_c$) & $\tau_\text{peak}$--$2\,\tau_\text{peak}$ & New parameter \\
\cutinhead{Accretion Disk}
 & Velocity dependence & $\lambda = 1/(1-v/c)$ & Doppler shifted per velocity bin \\
\cutinhead{BLR Disk-Wind}
 & Mean radius ($\bar{r}$) & 500--1200~$r_s$ & Narrower than 1D (500--2000~$r_s$) \\
\cutinhead{BLR Cloud}
 & (All parameters same as 1D) & \nodata & Velocity structure intrinsic to model \\
\enddata
\end{deluxetable*}
\endgroup

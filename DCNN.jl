...
function CNN_ensemble_2D(inputShape; maxChannels=32, dropout_rate=0.1, nPool=4, nDim=1) #this version works pretty well (train = 0.65, val = 1.07, ~30-40 epochs, learningRate ~1e-4)
    nT = inputShape[1]
    nLC = inputShape[2] 
    nC = inputShape[3]
    nBatch = inputShape[4]
    ...
    mediumFilter_t = max(5, convert(Int, floor(nT/50)))
    largeFilter_t = max(7, convert(Int, floor(nT/5)))
    smallFilter_v = max(2, convert(Int, floor(nLC/100)))
    mediumFilter_v = max(5, convert(Int, floor(nLC/50)))
    largeFilter_v = max(7, convert(Int, floor(nLC/5)))
    CNN_model = Chain(
        BatchNorm(nC, relu),
        Split(
            Chain(
